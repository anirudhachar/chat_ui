...
  const docInputRef = useRef<HTMLInputElement>(null);
  const debounceRef = useRef<NodeJS.Timeout | null>(null);
  //  NEW: input ref for WhatsApp keyboard control
  const inputRef = useRef<HTMLInputElement>(null);
  // ───────────────────────────────────────────────
  // WhatsApp-style Emoji  Keyboard Toggle
  // ───────────────────────────────────────────────
  const toggleEmojiPicker = () => {
    setShowEmojiPicker((prev) => {
      const next = !prev;
      next ? inputRef.current?.blur() : inputRef.current?.focus();
      return next;
    });
  };
  // ───────────────────────────────────────────────
...
  // ───────────────────────────────────────────────
  const handleSend = () => {
    if (recordedAudio) return sendRecordedAudio();
    if (!message.trim() && !linkPreview && !selectedFile) return;
    if (linkPreview) {
      onSendMessage(message.trim(), "link", {
        name: linkPreview.title,
...
            </>
          )}
          {/*  WhatsApp Emoji / Keyboard Toggle */}
          <button
            className={styles.iconButton}
            onClick={toggleEmojiPicker}
            type="button"
          >
            {showEmojiPicker ? "" : <FiSmile />}
          </button>
          <input
            ref={inputRef}
            type="text"
...
            onChange={(e) => {
              setMessage(e.target.value);
              setShowEmojiPicker(false); //  WhatsApp behavior
              onTyping?.();
            }}
            onKeyDown={handleKeyDown}
          />
